...
function SectionTitle({ label, title, description }: { label: string; title: string; description: string }) {
  return (
    <div className="mx-auto mb-8 max-w-2xl text-center md:mb-10">
      <p className="mb-2 text-xs font-semibold uppercase tracking-[0.32em] text-emerald-400">{label}</p>
...
function LinkButton({ href, children, variant = 'primary' }: { href: string; children: string; variant?: 'primary' | 'secondary' }) {
  const baseClass =
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className={`${baseClass} ${variantClass}`}>
      {children}
    </a>
  )
}
    ...
    <main className="bg-slate-950 text-slate-100">
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-8 px-4 py-14 md:grid-cols-2 md:items-center md:py-20">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-emerald-400">Club de Pádel Sacala x4</p>
            <h1 className="mt-4 text-4xl font-black leading-tight text-white md:text-6xl">Viví el pádel en Córdoba Capital</h1>
            <p className="mt-5 max-w-xl text-sm text-slate-300 md:text-base">
              Nos encontramos en Av Pueyrredón 2660 y abrimos también los domingos. Reservá rápido por WhatsApp y asegurá tu lugar.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <LinkButton href={links.reservas}>Reservar por WhatsApp</LinkButton>
        ...
        <div className="mx-auto grid max-w-6xl gap-6 px-4 md:grid-cols-3">
          {[
            ...
            },
            {
              ...
              detail: 'Canchas de pádel en un entorno moderno para entrenar y competir.',
            },
            {
              title: 'Turnos fijos',
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 md:py-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {['Cumpleaños', 'Torneos', 'Eventos privados'].map((eventType) => (
            <article key={eventType} className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
              <h3 className="text-xl font-semibold text-white">{eventType}</h3>
              <p className="mt-2 text-sm text-slate-300">Consultá opciones y armamos una propuesta para tu grupo.</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="border-t border-slate-800 bg-slate-900/40">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 md:grid-cols-2 md:items-start md:py-16">
          <div>
            ...
            <p className="mt-3 text-slate-300">Av Pueyrredón 2660, Córdoba Capital.</p>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-950 p-6">
            ...
            <p className="mt-3 text-sm text-slate-300">Respondemos rápido para reservas, turnos fijos y organización de eventos.</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <LinkButton href={links.reservas}>WhatsApp principal</LinkButton>
